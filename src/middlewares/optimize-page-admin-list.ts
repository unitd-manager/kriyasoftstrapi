...
const parsePageSize = (value: unknown): number | null => {
  ...
  return async (ctx: any, next: () => Promise<void>) => {
    try {
      ...
      if (isListPath) {
        ...
        const pageSize = parsePageSize(pagination.pageSize ?? query.pageSize);
        if (pageSize === null) {
          ctx.status = 400;
          ctx.body = {
      ...
      pagination.pageSize = parsePageSize(pagination.pageSize ?? query.pageSize);
      ...
      strapi.log.error(
        `[page-admin] middleware error correlationId=${correlationId}\n${errorDetails}`
      );
      // Don't call next() again here — return a 500 to avoid double-calling next()
      try {
        ctx.status = 500;
        ctx.body = { error: 'Internal server error', correlationId };
      } catch (e) {
        // ignore
      }
      return;
