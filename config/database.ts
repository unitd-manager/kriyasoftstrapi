...
const config = ({ env }: Core.Config.Shared.ConfigParams): Core.Config.Database => {
  ...
  const databaseSsl = env.bool('DATABASE_SSL', isProduction);
  const databaseSslRejectUnauthorized = env.bool('DATABASE_SSL_REJECT_UNAUTHORIZED', true);

  if (databaseSsl && !databaseSslRejectUnauthorized) {
    throw new Error('DATABASE_SSL_REJECT_UNAUTHORIZED=false is not allowed');
  }
